...
// ============================================================
// GOOGLE REVIEWS AUTO-MONITORING
// Add this to index.js BEFORE the WHATSAPP WEBHOOK section
// ============================================================

const GOOGLE_PLACES_API_KEY = process.env.GOOGLE_PLACES_API_KEY;

// ── FETCH & SYNC GOOGLE REVIEWS FOR A BUSINESS ─────────────
// POST /api/businesses/:id/sync-reviews
app.post("/api/businesses/:id/sync-reviews", async (req, res) => {
  try {
    ...
    // Get Place ID from Google Maps URL or stored place ID
    let placeId = business.google_place_id;

    if (!placeId && business.google_maps_url) {
      placeId = await getPlaceIdFromUrl(business.google_maps_url, business.business_name, business.city);
    }

    if (!placeId) {
      return res.status(400).json({ success: false, error: "No Google Place ID found. Please add your Google Maps URL in settings." });
    }

    // Save place ID if not already saved
    if (!business.google_place_id) {
      await supabase.from("businesses").update({ google_place_id: placeId }).eq("id", id);
    }
    ...
    if (!reviews || reviews.length === 0) {
      return res.json({ success: true, synced: 0, message: "No reviews found on Google yet." });
    }

    // Save new reviews to DB (skip duplicates)
    let syncedCount = 0;
    ...
    res.json({
      ...
      message: syncedCount > 0 ? `${syncedCount} new reviews synced from Google!` : "Already up to date."
    });
...
// ── GET PLACE DETAILS (rating + review count) ──────────────
// GET /api/businesses/:id/google-rating
app.get("/api/businesses/:id/google-rating", async (req, res) => {
  try {
    ...
    if (!placeId && business.google_maps_url) {
      placeId = await getPlaceIdFromUrl(business.google_maps_url, business.business_name, business.city);
      if (placeId) await supabase.from("businesses").update({ google_place_id: placeId }).eq("id", id);
    }
...
async function getPlaceIdFromUrl(mapsUrl, businessName, city) {
  try {
    ...
    // Search by business name using Places API
...
async function fetchGoogleReviews(placeId) {
  try {
    // Use Places API (New) — returns up to 53 reviews
    const response = await axios.get(
    ...
    return reviews.map(r => ({
      google_review_id: `${placeId}_${r.publishTime}`,
      reviewer_name: r.authorAttribution?.displayName || 'Anonymous',
      rating: r.rating,
      review_text: r.text?.text || '',
      review_date: new Date(r.publishTime).toISOString(),
      reviewer_photo: r.authorAttribution?.photoUri || null
    }));
    ...
    try {
      ...
      return reviews.map(r => ({
        google_review_id: `${placeId}_${r.time}`,
...
// ── HELPER: Get Place Details (rating + count) ──────────────
  ...
  try {
    ...
    for (const business of (businesses || [])) {
      try {
        ...
        if (!placeId && business.google_maps_url) {
          placeId = await getPlaceIdFromUrl(business.google_maps_url, business.business_name, business.city);
          if (placeId) await supabase.from("businesses").update({ google_place_id: placeId }).eq("id", business.id);
        }
        if (!placeId) continue;
        ...
        for (const review of reviews) {
          ...
          if (!existing) {
            ...
            // Alert business owner about new review
        ...
        if (newCount > 0) {
          console.log(`Synced ${newCount} new reviews for ${business.business_name}`);
          await updateAnalytics(business.id);
        }
      } catch (e) {
        console.error(`Sync error for ${business.business_name}:`, e.message);
      }
    }
  } catch (err) {
