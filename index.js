...
// ============================================================
// MODULE 1: AI BUSINESS ASSISTANT
// ============================================================
app.post("/api/ai-assistant", async (req, res) => {
  try {
    const { message, business_name, business_category, business_city, conversation = [] } = req.body;
    if (!message) return res.status(400).json({ success: false, error: "Message required" });

    const systemPrompt = `You are an expert AI Business Assistant built specifically for Indian businesses. You work for ${business_name || "an Indian business"}, a ${business_category || "business"} based in ${business_city || "India"}.

Your expertise covers:
- Indian business law (contracts, NDAs, agreements under Indian Contract Act)
- GST filing, compliance, deadlines, and penalties
- ROC filings, company law (Companies Act 2013)
- PF, ESI, labour compliance
- MSME registration, Udyam portal
- Government schemes: PM Vishwakarma, CLCSS, CGTMSE, PMEGP, Stand-Up India, Startup India
- State-specific schemes (especially Gujarat: iNDEXTb, Vibrant Gujarat incentives)
- Export benefits: RODTEP, MEIS, SEIS, Advance Authorisation
- Marketing strategies for Indian SMEs
- Business documents: invoices, quotations, proposals
- Banking: working capital loans, CC limits, MUDRA loans
- Income tax for businesses: presumptive taxation, Section 44AD/44ADA

Rules:
- Always give India-specific, practical advice
- Use Indian examples, Indian Rupees, Indian city/state context
...
- Format responses clearly with bullet points or numbered lists when helpful`;
    ...
    const response = await axios.post("https://api.anthropic.com/v1/messages", { model: "claude-sonnet-4-6", max_tokens: 1000, system: systemPrompt, messages }, { headers: { "x-api-key": ANTHROPIC_KEY, "anthropic-version": "2023-06-01", "Content-Type": "application/json" } });
