...
// ============================================================
async function scrapeGoogleRating(mapsUrl, businessName, city) {
  // Pull REAL competitor data from the official Google Places API.
  // No fabricated ratings: if the place can't be resolved, return null so the UI honestly shows "Not rated".
  try {
    const placeId = await getPlaceIdFromUrl(mapsUrl, businessName, city);
    if (!placeId) return null;
    const details = await getPlaceDetails(placeId);
    if (!details || details.google_rating == null) return null;
    return {
      rating: details.google_rating,
      review_count: details.google_review_count || 0,
      category: null,
      recent_reviews: []
    };
  } catch (err) {
    console.error("scrapeGoogleRating error:", err.message);
    return null;
  }
}
